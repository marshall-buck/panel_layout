...
* **Refactor (Breaking)**: `PanelThemeData.headerHeight` has been removed.
* **Feat**: Added `PanelThemeData.headerPadding`. Header height is now dynamically calculated as `iconSize + (headerPadding * 2)` to better support responsive layouts and align with rail sizing logic.
* **Feat**: `BasePanel` now supports overriding the calculated header height via the existing `headerHeight` property, but defaults to the dynamic calculation.
