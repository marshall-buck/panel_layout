...
void main() {
  group('PanelArea', () {
    ...
    testWidgets('detached panel is not rendered', (tester) async {
      layoutController.registerPanel(
        const PanelId('inline'),
        builder: (context, _) => const Text('Inline Panel'),
        sizing: const FlexibleSizing(1),
        mode: PanelMode.inline,
        anchor: PanelAnchor.left,
      );
      layoutController.registerPanel(
        const PanelId('detached'),
        builder: (context, _) => const Text('Detached Panel'),
        sizing: const FixedSizing(100),
        mode: PanelMode.detached,
        anchor: PanelAnchor.left,
      );
      ...
      await tester.pumpWidget(
        Directionality(
          ...
          child: PanelArea(
            panelLayoutController: layoutController,
            panelIds: const [PanelId('inline'), PanelId('detached')],
          ),
        ),
      );

      expect(find.text('Inline Panel'), findsOneWidget);
      expect(find.text('Detached Panel'), findsNothing);
    });
  });
}
