...
void main() {
  group('Panel Resize Constraints', () {
    testWidgets('Cannot resize a collapsed panel', (tester) async {
      ...
      final containerFinder = find.byWidgetPredicate((widget) => widget is Container && widget.color == Colors.red);
      final size = tester.getSize(containerFinder);
      
      expect(size.width, 200.0, reason: "Panel size should remain 200 after attempting resize while collapsed");
    });

    testWidgets('Cannot resize panel smaller than collapsed size (rail width)', (tester) async {
       final panelId = PanelId('sidebar');
