...
class TestFixedPanel extends BasePanel {
  TestFixedPanel({super.key, required String id, required double width, super.mode, super.anchor, super.anchorTo})
    : super(
        id: PanelId(id),
        width: width,
...
void main() {
  ...
  testWidgets('Anchored overlay panel animates out instead of disappearing instantly', (tester) async {
    final controller = PanelLayoutController();
    ...
    final Size midSize = tester.getSize(find.descendant(
      of: find.byType(AnimatedPanel).last, 
      matching: find.byType(ClipRect),
    ));
    
    expect(midSize.width, greaterThan(0.0), reason: 'Panel width should be > 0 during animation');
    expect(midSize.width, lessThan(100.0), reason: 'Panel width should be < 100 during animation');
