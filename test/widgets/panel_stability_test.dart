...
void main() {
  ...
  group('Directional Stability Tests', () {
    testWidgets('Anchor Right: Right neighbor (Next) stays stable', (
      tester,
    ) async {
      ...
      expect(
        midC,
        closeTo(initialC, 0.01),
        reason:
            'Neighbor C (Right) should not move/resize when B (Anchor Right) collapses',
      );
    });

    testWidgets('Anchor Left: Left neighbor (Prev) stays stable', (
      tester,
    ) async {
      final controller = PanelAreaController();
      ...
      expect(
        midA,
        closeTo(initialA, 0.01),
        reason:
            'Neighbor A (Left) should not move/resize when B (Anchor Left) collapses',
      );
    });

    testWidgets('Anchor Bottom: Bottom neighbor (Next) stays stable', (
      tester,
    ) async {
      final controller = PanelAreaController();
      // [Flex A]
      ...
      expect(
        ...
        closeTo(initialC, 0.01),
        reason:
            'Neighbor C (Bottom) should not move/resize when B (Anchor Bottom) collapses',
      );
    });
    ...
    testWidgets('Anchor Top: Top neighbor (Prev) stays stable', (tester) async {
      ...
      expect(
        ...
        closeTo(initialA, 0.01),
        reason:
            'Neighbor A (Top) should not move/resize when B (Anchor Top) collapses',
      );
    });
  });
}
